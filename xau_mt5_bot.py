...
# On MT5 spot gold this is tick volume, matching TradingView. Re-tune the
# threshold for THIS feed: override at runtime with XAU_VOL_THR / XAU_VOL_FILTER.
VOLUME_FILTER    = _env_bool("XAU_VOL_FILTER", True)
VOLUME_THRESHOLD = _env_float("XAU_VOL_THR", 20)
VOLUME_FAST      = 5
VOLUME_SLOW      = 10
...
# server is AHEAD of UTC (IC Markets ~ +2 winter / +3 summer). Override with
# XAU_TZ_OFFSET so the backtest session gate lines up with real UTC hours.
SERVER_UTC_OFFSET = _env_float("XAU_TZ_OFFSET", 0)
