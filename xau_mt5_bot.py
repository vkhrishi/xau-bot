...
# Env helpers so the tunable knobs can be overridden per-run without editing.
...
INTERVAL     = "15m"
CAPITAL_USDT = _env_float("XAU_CAPITAL", 200)   # Account balance for risk sizing.

# -- Risk --
RISK_PER_TRADE_PCT     = _env_float("XAU_RISK_PCT", 1.0)
...
# On MT5 spot gold this is tick volume, matching TradingView. Calibrated on the
# IC Markets feed: threshold ~3 replicates LuxAlgo's 20 on TradingView's scale.
# Override per-run with XAU_VOL_THR / XAU_VOL_FILTER.
VOLUME_FILTER    = _env_bool("XAU_VOL_FILTER", True)
# On MT5 spot gold this is tick volume, matching TradingView. Calibrated on the
# IC Markets feed: threshold ~3 replicates LuxAlgo's 20 on TradingView's scale.
# Override per-run with XAU_VOL_THR / XAU_VOL_FILTER.
VOLUME_FILTER    = _env_bool("XAU_VOL_FILTER", True)
VOLUME_THRESHOLD = _env_float("XAU_VOL_THR", 3)
VOLUME_FAST      = 5
VOLUME_SLOW      = 10
...
# Safety rail: never silently over-risk. If the broker MIN lot risks more than
# MAX_RISK_USDT * tolerance, SKIP the trade instead of taking an oversized
# position (the small-account trap on gold). Disable with XAU_RISK_HARDCAP=0.
RISK_HARD_CAP      = _env_bool("XAU_RISK_HARDCAP", True)
RISK_CAP_TOLERANCE = _env_float("XAU_RISK_TOL", 1.10)   # allow +10% for lot granularity

# -- Session (UTC): London + NY overlap --
SESSION_START_UTC = (7, 0)
SESSION_END_UTC   = (15, 0)
TRADE_WEEKENDS    = False
# MT5 candle timestamps are in BROKER SERVER time. This is how many hours the
# server is AHEAD of UTC (IC Markets ~ +2 winter / +3 summer). If XAU_TZ_OFFSET
# is not set, the bot auto-detects it from the live server clock (handles DST).
SERVER_UTC_OFFSET = _env_float("XAU_TZ_OFFSET", 3)
_TZ_ENV_SET = "XAU_TZ_OFFSET" in os.environ
...
BACKTEST_SESSION_ONLY  = _env_bool("XAU_SESSION_ONLY", True)
...
def detect_server_utc_offset():
    """Hours the broker server clock is ahead of UTC, read from a live tick.
    Auto-handles DST. Falls back to the configured SERVER_UTC_OFFSET on failure."""
    try:
        tick = mt5.symbol_info_tick(SYMBOL)
        if tick and tick.time:
            server_now = datetime.datetime.utcfromtimestamp(tick.time)
            return round((server_now - utc_now()).total_seconds() / 3600.0)
    except Exception:
        pass
...
def calculate_position(direction, entry_price, grab_low, grab_high, sym):
    ...
    if per_lot_risk <= 0:
        return None
    ideal_lots = MAX_RISK_USDT / per_lot_risk
    lots = round_lots(min(ideal_lots, sym["volume_max"]), sym["volume_step"])
    if lots < sym["volume_min"]:
        lots = sym["volume_min"]   # broker floor; may exceed the risk cap

    risk_usdt = lots * sym["contract_size"] * sl_distance
    # Safety rail: refuse to over-risk. If even the minimum lot risks more than
    # the cap, skip the trade rather than take an oversized position.
    if RISK_HARD_CAP and risk_usdt > MAX_RISK_USDT * RISK_CAP_TOLERANCE:
        logging.warning(
            "SKIP %s: min lot %.2f risks $%.2f > cap $%.2f (fund a larger account "
            "or raise RISK_PER_TRADE_PCT).",
            direction, lots, risk_usdt, MAX_RISK_USDT)
        return None
...
def _replay(candles, sym):
    ...
    while i < n - 1:
        ...
        if BACKTEST_SESSION_ONLY and not in_session(
                ...
                - datetime.timedelta(hours=SERVER_UTC_OFFSET)):
            i += 1
            continue
        entry_idx = None
...
def run_backtest(days=90):
    global SERVER_UTC_OFFSET
    if not _TZ_ENV_SET:
        SERVER_UTC_OFFSET = detect_server_utc_offset()
        logging.info("Server-UTC offset auto-detected: +%g h (%s)",
                     SERVER_UTC_OFFSET, SYMBOL)
